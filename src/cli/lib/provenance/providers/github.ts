...
import type { Enable, GenerateProvenance } from "../base";
...
export const generateProvenance: GenerateProvenance =
  async function generateProvenance(subjects) {
    ...
    try {
      ...
      throw new CliError(
        message`Failed to generate provenance for ${values(subjects.map((subject) => subject.name))}`,
        { cause },
      );
    }
  };
